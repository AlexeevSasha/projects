...
export const FilterPrimaryOrderByPeriod = () => {
  const { locale } = useRouter();
  const lang = getLanguage(locale);
  const { filterData, setFilterData } = useContext(PrimaryOrderFilterContext);
  const [showDate, setShowDate] = useState(false);
  const [newDate, setNewDate] = useState<Pick<IPrimaryOrderSearchParams, "startDate" | "endDate">>({
    startDate: null,
    endDate: null,
  });
  ...
  return (
    <div className="mt-4 flex items-center justify-between gap-2">
      <h1>{lang.table.filter_by_period}</h1>
