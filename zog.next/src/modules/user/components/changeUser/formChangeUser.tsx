...
interface IProps {
  ...
  setUser: (cb: (value: object) => object) => void;
  onClose: () => void;
}

export const FormChangeUser = ({ onSubmit, setUser, onClose }: IProps) => {
    ...
    <form onSubmit={onSubmit} className="relative flex-auto p-6">
      <div className={"mb-5"}>
        <label className="mb-2 block text-sm font-medium text-gray-900 dark:text-white">
          {lang.modal.user.enter_new_email}
          <small className={"text-red ml-1"}> * {lang.modal.user.required}</small>
        </label>
        <input
          type={"email"}
          required
