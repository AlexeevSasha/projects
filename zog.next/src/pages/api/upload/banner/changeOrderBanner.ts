...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST") {
    ...
    try {
      const { banners } = req.body as { banners: IBanner[] };

      const bannersChange = banners.map((el) =>
      ...
      await Promise.all(bannersChange);

      res
        .status(200)
        .json(
          ResponseHandler.onSuccess({ message: "Порядок баннеров успешно обновлён", data: banners })
        );
    } catch (e) {
