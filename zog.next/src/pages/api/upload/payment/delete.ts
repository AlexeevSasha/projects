...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "DELETE") {
    ...
    try {
      const banner = await prisma.paymentBanners.findFirst({ where: { id: String(id) } });
      ...
      await new UploadFileGoogleDrive().delete(banner.image_id);
      await prisma.paymentBanners.delete({ where: { id: banner.id } });
      res
        .status(200)
