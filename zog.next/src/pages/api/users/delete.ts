...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "DELETE") {
    ...
    try {
      ...
      res.status(200).json(ResponseHandler.onSuccess({ message: "Пользователь удален" }));
    } catch (e) {
      res.status(400).json(ResponseHandler.onError({ message: "Не удалось удалить пользователя" }));
    }
  } else {
