...
import { clubsStaffActions } from "store/clubsStaff/clubsStaff";
import { draftClubsStaff, getClubsStaffById, publishClubsStaff } from "store/clubsStaff/clubsStaffActionAsync";
import { clubsStaffSelector } from "store/clubsStaff/clubsStaffSelectors";
...
export const ClubsStaffForm = memo(() => {
  ...
  const staff = useSelector(clubsStaffSelector);
  ...
  const submitForm = async (draft?: boolean) => {
    ...
    dispatch(
      (draft ? draftClubsStaff : publishClubsStaff)(deepMerge<Staff>({ ...values, Id: id }, form.getFieldsValue()))
    )
      .unwrap()
      .then(() => {
        dispatch(
  ...
  useEffect(() => {
    if (!isCreate && id) {
      dispatch(getClubsStaffById(id)).unwrap().then(setValues);
    }

    return () => {
      dispatch(clubsStaffActions.resetStaff());
    };
  }, [id]);
    ...
    <Drawer
      ...
      footer={
        <Footer>
          ...
          <Button
            onClick={() => (isCreate || staff?.Status === "Draft" ? showConfirm : submitForm)()}
            type="primary"
            htmlType="submit"
          >
            {isCreate || staff?.Status === "Draft" ? t("allPages.buttonsText.publish") : t("allPages.buttonsText.save")}
          </Button>
        </Footer>
      }
    >
      ...
      {!isCreate && !staff ? (
        ...
        <Form form={form} layout="vertical" requiredMark={false} initialValues={staff} validateTrigger="onBlur">
          {staff && (
            <Status>
              {t("allPages.status")}:{" "}
              <Tag color={statusColors[staff?.Status || "None"]}>
                {t(`allPages.statuses.${staff?.Status || "None"}`)}
              </Tag>
            </Status>
          )}
