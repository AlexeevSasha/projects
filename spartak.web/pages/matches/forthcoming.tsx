  ...
  return (
    <>
      {matchCalendar?.length ? (
        <>
          <MatchSlider typeMatch={"new"} matchCalendar={matchCalendar} />

          <TableMatches typeMatch="new" matchCalendar={matchCalendar} teamId={teamId} votings={votings} />
        </>
      ) : (
        <EmptyTableMatches
          title={lang[locale].profile.matches.emptyTitle}
          text={lang[locale].profile.matches.noUpcomingMatches}
          imgSrc={Ball.src}
        />
      )}
    </>
...
export const getServerSideProps: GetServerSideProps = async ({ query, res }) => {
  isDev && console.log("\r\n\r\n", "\x1b[34m", "start", "/matches/forthcoming", new Date());
  res.setHeader(
    "Cache-Control",
    `public, s-maxage=${process.env.NEXT_PUBLIC_REVALIDATE_MIN}, stale-while-revalidate=59`
  );

  const { teams = [], metaTags } = await getInitialData({ pathname: "/matches/forthcoming" });
  ...
  const matchCalendar = season
    ? await matchRepository
      .fetchCalendar({
        ...
        seasonId: season?.Id,
        matchListType: "Future",
        Status: "Published",
      })
      .then((res) => res.sort((current, next) => new Date(current.Date).getTime() - new Date(next.Date).getTime()))
      .catch(() => null)
    : [];

  isDev && console.log("\x1b[34m", "done", "/matches/forthcoming", new Date(), "\r\n");
