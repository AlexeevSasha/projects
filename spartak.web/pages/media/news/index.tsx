...
import type { ITeam } from "../../../src/api/dto/ITeam";
...
export default function News({ categoryList, teams, year, month, teamId, bannerSpartak, bannerSpartakYouth }: IProps) {
    ...
    () => [
      ...teams.map((item) => {
        return {
          ...
          text: getLocalValue(item.ShortName, locale),
        };
      }),
    ],
    [query, teams]
  ...
  return (
    <>
      ...
      <MainBannerWithTitle
        title={lang[locale].header.navList["media/news"]}
        banner={isYouthCommand() ? getLocalValue(bannerSpartakYouth, "ru") : getLocalValue(bannerSpartak, "ru")}
      />
      <MediaNavMenu subMenuList={mediaMainSubMenu} categoryList={categoryList} />
      <MediaList mediaType="News" section="Site" teamId={teamId} />
    </>
  );
}
...
export const getServerSideProps: GetServerSideProps = async ({ query, res }) => {
  ...
  const [categoryRes, teamRes] = await Promise.allSettled([
    mediaRepository.fetchMediaCategories({ section: "Site", mediaType: "News" }),
  ...
  const response = (await cmsRepository.fetchCms({ Type: "mediaNews" })).value[0];
  ...
  return {
    props: {
      ...
      month,
      teamId,
      metaTags: enterData?.metaTags || (seodata as Record<string, any>)["/media/news"],
