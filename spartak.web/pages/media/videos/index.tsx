...
export default function Videos({ categoryList, teams, year, month, teamId }: IProps) {
  const { locale = "ru", query, push, replace } = useRouter();
  ...
  // Редирект на страницу с заполненными query параметрами
  useEffect(() => {
  ...
  const renderCategoryVideos = useMemo(() => {
    const categoryInfo = categoryList.find((category) => category.Id === query.category);
    return <MediaList section="Site" mediaType="Video" categoryInfo={categoryInfo} teamId={teamId} />;
  }, [categoryList, query, locale]);
  ...
  return (
    <>
      ...
      <MainBannerWithTitle
        title={lang[locale].header.navList["media/videos"]}
...
export const getServerSideProps: GetServerSideProps = async ({ query, res }) => {
  ...
  const [categoryRes, teamRes] = await Promise.allSettled([
    mediaRepository.fetchMediaCategories({ section: "Site", mediaType: "Video" }),
  ...
  return {
    props: {
      ...
      month,
      teamId,
      metaTags: (seodata as Record<string, any>)["/media/videos"],
    },
  };
};
