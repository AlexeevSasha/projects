...
export const PromoCard = ({ promo }: Props) => {
  ...
  return (
    <Link prefetch={false} href={`${pathname}/${promo.Id}`} passHref>
      <Container>
        <NewsImgContainer
          isActive={
            !(
              (promo.IsWinner || promo?.OutOfStock || new Date(promo.EndDate) < new Date()) &&
              promo.LoyaltyAwardType !== "ExternalReference"
            )
          }
        >
          <NextImage src={promo.ImageUrl} alt={promo.Title} objectFit="cover" />
        </NewsImgContainer>
