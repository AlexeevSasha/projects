...
const breadcrumbs = [
  { title: "Главная", link: "" },
  { title: "Справочник заболеваний ", link: "" },
];
...
export default function DiseasePage(props: Props) {
  return (
    <>
      <ContainerArticle>
        ...
        <ContainerTitle>
          <TitleH1 smallText={`${props.disease.quantity} товаров`} title={props.disease.name} />
          <ContainerShare>
            <Share />
          </ContainerShare>
        </ContainerTitle>
      ...
      <ContainerContent>
        ...
        <div>
          {props.disease.text ? (
            <Text
              dangerouslySetInnerHTML={{
                __html: XSSProtection(props.disease.text.replace(/(\r\n|\r|\n)/g, "<br>")),
              }}
            />
          ) : null}
          <ContainerTags>
          ...
          <ContainerCustom padding={"xxsm"}>
            <Product.ProductsGrid hidePagination products={props.disease?.products} />
          </ContainerCustom>
          <ContainerCustom padding={"sm"} id={"instructions-for-use"} paddingMb={"md"}>
            <MetaCatalogue.InstructionsUseMedicinalProduct
              shortDescription={props.disease.shortDescription}
              certificate={props.disease.certificate}
              images={props.disease.images}
              descriptionSections={props.disease.descriptionSections}
...
DiseasePage.getLayout = getLayout;

export const getServerSideProps: GetServerSideProps = async () => {
  try {
    const { metaTags = {}, ...initialData } = await getInitialData({
      pathname: "/product/metacatalogue/disease",
    });
    const [filters, disease, recommendationProducts] = await Promise.allSettled([
      gerMetaCatalogueFilter(),
      getMetaCatalogue("disease"),
      getProducts({}),
    ]);
    ...
    return {
      props: {
        ...
        disease: disease.status === "fulfilled" ? disease.value : {},
