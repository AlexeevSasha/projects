...
const breadcrumbs = [
  { title: "Главная", link: "" },
  { title: "Действующие вещества ", link: "" },
];
...
export default function ActiveIngredientPage(props: Props) {
  return (
    <>
      <ContainerArticle>
        ...
        <ContainerTitle>
          <TitleH1
            smallText={`${props.activeIngredient.quantity} товаров`}
            title={props.activeIngredient.name}
          />
          <ContainerShare>
            <Share />
          </ContainerShare>
      ...
      <ContainerContent>
        ...
        <div>
          {props.activeIngredient.text ? (
            <Text
              dangerouslySetInnerHTML={{
                __html: XSSProtection(props.activeIngredient.text.replace(/(\r\n|\r|\n)/g, "<br>")),
              }}
            />
          ) : null}
          <ContainerTags>
          ...
          <ContainerCustom padding={"xxsm"}>
            <Product.ProductsGrid hidePagination products={props.activeIngredient?.products} />
          </ContainerCustom>
          <ContainerCustom padding={"sm"}>
            <MetaCatalogue.DescriptionsProduct
              descriptionSections={props.activeIngredient.descriptionSections}
            />
          </ContainerCustom>
...
ActiveIngredientPage.getLayout = getLayout;

export const getServerSideProps: GetServerSideProps = async () => {
  try {
    const { metaTags = {}, ...initialData } = await getInitialData({
      pathname: "/product/metacatalogue/active-ingredient",
    });
    const [filters, activeIngredient, recommendationProducts, recommendationCategory, sales] =
      await Promise.allSettled([
        gerMetaCatalogueFilter(),
        getMetaCatalogue("active-ingredient"),
        getProducts({}),
    ...
    return {
      props: {
        ...
        activeIngredient: activeIngredient.status === "fulfilled" ? activeIngredient.value : {},
