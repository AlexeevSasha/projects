...
export default function ManufacturerPage(props: Props) {
  return (
    <>
      <ContainerArticle>
        ...
        <ContainerTitle>
          <TitleH1
            smallText={`${props.manufacturer.quantity} товаров`}
      ...
      <ContainerContent>
        ...
        <div>
          {props.manufacturer.text ? (
            <Text
              dangerouslySetInnerHTML={{
                __html: XSSProtection(props.manufacturer.text.replace(/(\r\n|\r|\n)/g, "<br>")),
              }}
            />
          ) : null}
          <ContainerTags>
          ...
          <ContainerCustom padding={"xxsm"}>
            <Product.ProductsGrid hidePagination products={props.manufacturer?.products} />
          </ContainerCustom>
          <ContainerCustom padding={"sm"}>
            <MetaCatalogue.DescriptionsProduct
              descriptionSections={props.manufacturer.descriptionSections}
            />
          </ContainerCustom>
...
ManufacturerPage.getLayout = getLayout;

export const getServerSideProps: GetServerSideProps = async () => {
  try {
    const { metaTags = {}, ...initialData } = await getInitialData({
      pathname: "/product/metacatalogue/manufacturer",
    });
    const [filters, manufacturer, recommendationProducts, sales] = await Promise.allSettled([
      gerMetaCatalogueFilter(),
      getMetaCatalogue("manufacturer"),
      getProducts({}),
      getSalesList({}),
    ]);
    ...
    return {
      props: {
        ...
        manufacturer: manufacturer.status === "fulfilled" ? manufacturer.value : {},
