...
import { getBiletkaData, getWinlineData } from "../../../../api/requests/matches";
...
export const MatchForm = ({ idEntityChanged, onClose, visible }: IProps) => {
  ...
  const winlineData = useGetMatchData(searchDate, getWinlineData);

  useEffect(() => {
    if (matchForChanged) {
      form.setFieldsValue({
        statisticEventId: matchForChanged.statisticEventId,
        winlineOddsEventId: matchForChanged.winlineOddsEventId,
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical">
        ...
        <Form.Item label={t("matches.winline")} name={"winlineOddsEventId"} initialValue={matchForChanged?.winlineOddsEventId}>
          <Select placeholder={t("common.selectPlaceholder")} showSearch={false} size={"middle"} allowClear>
            {winlineData.map((match) => (
