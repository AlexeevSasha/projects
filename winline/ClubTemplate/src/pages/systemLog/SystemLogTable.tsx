...
export const SystemLogTable = ({ access }: { access: boolean }) => {
  ...
  const [filtersLog, setFilterValues] = useState<ISystemLogFilters>({
    name: "",
    employee: undefined,
    date: null,
    sorting: "",
  ...
  useEffect(() => {
    if (validationSize(filtersLog.name, 256)) {
      message.error(t("validations.filtersSizeError"));
    } else {
      dispatch(getAllSystemLog(filtersLog));
    }
  }, [filtersLog.sorting, filtersLog.pagination, filtersLog.date, filtersLog.name, filtersLog.employee]);
      ...
      <ContentStyled>
        <Card>
          <FiltersHeaderWithDate
            ...
            selectName="employee"
            selectValue={filtersLog.employee}
          ...
          <Table
            ...
            onChange={(pagination, filters, sorter: SorterResult<ISystemLogItem> | SorterResult<ISystemLogItem>[]) =>
              onChangeDataTable<ISystemLogItem, ISystemLogFilters>(pagination, sorter, filtersLog, setFilterValues)
            }
            pagination={onChangePaginationTable(count, filtersLog.pagination)}
            scroll={{ x: 1120 }}
            sticky
          />
        </Card>
