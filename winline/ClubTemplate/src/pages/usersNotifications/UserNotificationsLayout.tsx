        ...
        <Card>
          <Tabs onChange={handleTabChange} defaultActiveKey={tabKey} tabBarExtraContent={access && tabKey == "wait" && button}>
            <TabPane tab={t("users.notifications.linksText.wait")} key={tabs[0]}>
              {tabKey === tabs[0] && (
                <NotificationsTable access={access} isHistory={false} setShowForm={setShowForm} showForm={showForm} cities={citiesData} />
              )}
            </TabPane>
            <TabPane tab={t("users.notifications.linksText.history")} key={tabs[1]}>
              {tabKey === tabs[1] && <NotificationsTable access={access} isHistory={true} cities={citiesData} />}
            </TabPane>
          </Tabs>
        </Card>
      </ContentStyled>
