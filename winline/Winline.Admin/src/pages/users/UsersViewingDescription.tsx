...
export const UsersViewingDescription = ({ userDescription, onClose, cities, clubs }: IProps) => {
    ...
    >
      <Descriptions column={1}>
        ...
        <Descriptions.Item label={t("common.project")}>
          {clubs.find((club) => club.id === userDescription?.clubId)?.clubName}
        </Descriptions.Item>
        <Descriptions.Item label={t("users.registerAgree")}>{t("common.yes")}</Descriptions.Item>
      </Descriptions>
    </Drawer>
  );
};
