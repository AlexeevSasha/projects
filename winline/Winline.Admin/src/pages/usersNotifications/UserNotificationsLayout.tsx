...
import { getCitiesThunk, getClubsThunk } from "../../modules/commons/commonsActionAsync";
...
export const UserNotificationsLayout = ({ access }: { access: boolean }) => {
  ...
  const { clubs, cities } = useSelector((state: StateType) => state.commons);
  ...
  const projectsData = useMemo(
    () =>
      clubs.map((topic: IClub) => {
        return { value: topic.id, label: topic.clubName };
      }),
    [clubs]
  );
        ...
        <Card>
          <Tabs onChange={handleTabChange} defaultActiveKey={tabKey} tabBarExtraContent={access && tabKey == "wait" && button}>
            ...
            <TabPane tab={t("users.notifications.linksText.history")} key={tabs[1]}>
              {tabKey === tabs[1] && (
                <NotificationsTable access={access} isHistory={true} showForm={showForm} projects={projectsData} cities={citiesData} />
              )}
            </TabPane>
          </Tabs>
        </Card>
