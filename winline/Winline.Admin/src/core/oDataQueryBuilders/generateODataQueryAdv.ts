...
export const generateODataQueryAdv = (filterData: IAdvFilters | undefined) => {
  ...
  if (filterData.name || filterData.status || filterData.clubId || filterData.author) {
    const filters: string[] = [];

    if (filterData.name) {
      filters.push(`contains(tolower(name), '${encodeURIComponent(filterData.name.split(" ")[0].toLowerCase())}')`);
    }
    ...
    if (filterData.clubId) {
      filters.push(`clubId eq ${filterData.clubId}`);
    }
