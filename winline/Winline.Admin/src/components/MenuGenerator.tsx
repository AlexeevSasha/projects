      ...
      menu.children ? (
        <SubMenuStyles key={"sub-" + menu.path} title={t(menu.title)} icon={menu.icon}>
          {menu.children
            ?.filter((item) => MenuItemValidatorPolicy(item.policy))
            .map((submenu: IMenu) => (
              <MenuItemStyles disabled={submenu.disabled} key={submenu.path} icon={submenu.icon} onClick={() => changeRoute(submenu.path)}>
                {t(submenu.title)}
              </MenuItemStyles>
            ))}
        </SubMenuStyles>
      ) : (
        <MenuItemStyles disabled={menu.disabled} key={menu.path} icon={menu.icon} onClick={() => changeRoute(menu.path)}>
          {t(menu.title)}
        </MenuItemStyles>
      )
    );
