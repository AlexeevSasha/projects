...
export const generateColumnsEmployees = (access: boolean, handlers: IHandler): ColumnsType<IEmployee> => {
            ...
            <Tooltip placement="top" title={handlers.translation("employees.table.columns.activationDate.tooltip")}>
              {access && (
                <Button
                  ...
                  icon={<MailOutlined />}
                  type={"primary"}
                  disabled={handlers.disabledButtonsInvitation[id]}
                />
              )}
            </Tooltip>
          </ColumnActivated>
  ...
  const columnActions: ColumnType<IEmployee> = access
    ? {
  ...
  return [columnName, columnEmail, columnRole, columnDateActivated, columnDate, columnActions];
};
