...
export const SystemLogDescription = ({ descriptionLog, onClose }: IProps) => {
  ...
  console.log(descriptionLog?.dataChanges?.length && Object.keys(descriptionLog.dataChanges?.[0])[0]);

  return (
    <Drawer
      title={`${t("common.id")}: ${descriptionLog?.id}`}
      closable={true}
      destroyOnClose={true}
      onClose={onClose}
      ...
      <Descriptions column={1}>
        ...
        <Descriptions.Item label={t(typeFields[`${descriptionLog?.actionName}`])}>
          {descriptionLog?.dataChanges && typeof descriptionLog.dataChanges !== "string" ? (
            <div>
              {descriptionLog.dataChanges instanceof Array && Object.keys(descriptionLog.dataChanges?.[0])[0] !== 'Key'
                ...
                : Object.entries(descriptionLog.dataChanges).map(([column, value]) => {
                  ...
                  return (
                    <p key={column} style={{ display: 'flex' }}>
                      <Text type="secondary">{`${column}:`}</Text>
                      <div>
