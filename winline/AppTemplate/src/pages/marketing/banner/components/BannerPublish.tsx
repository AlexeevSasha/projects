...
export default ({ access, selectedForm, showBannerForm, visible }: IProps) => {
  ...
  const showUpdateModal = (key: string, entity: IBanner) => {
    Modal.confirm({
      ...
      maskClosable: true,
      content: t("marketing.banner.modal.hidden"),
  ...
  const resetFilters = useCallback(() => {
    setFilterValues({ ...filterValues, name: "", contentStatus: "Published", date: null });
  }, [setFilterValues, filterValues]);

  const columns = generateColumnsBanner(true, access, {
    showBannerForm,
    showUpdateModal,
