...
export const InputUserGroup = React.memo(({ access, saveProposal, info, options, disabled, clickEdit }: IProps) => {
  const defValue = info.userGroups.map((item) => {
  ...
  return (
    <InputContainer>
      <Title>Группа пользователей</Title>

      <SelectContainer id={"userGroups"}>
        ...
        {access === Access.Edit ? (
          disabled ? (
            <IconContainerFloatingmes title={ru.floatingmes.edit} id={"edit_user_group"} onClick={onClickIconEdit}>
              <IconEdit />
            </IconContainerFloatingmes>
          ) : (
            <>
              <IconContainerFloatingmes title={ru.floatingmes.save} id={"save_user_group"} onClick={onSave}>
                <IconSave />
              </IconContainerFloatingmes>
              <IconContainerFloatingmes
                title={ru.floatingmes.cancel}
                id={"close_edit_user_group"}
